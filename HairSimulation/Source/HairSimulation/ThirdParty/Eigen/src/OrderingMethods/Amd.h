...
void minimum_degree_ordering(SparseMatrix<Scalar,ColMajor,StorageIndex>& C, PermutationMatrix<Dynamic,Dynamic,StorageIndex>& perm)
{
  ...
  StorageIndex* llast    = perm.indices().data();                              /* use P as workspace for last */
  ...
  while (nel < n)                         /* while (selecting pivots) do */
  {
    ...
    for(k1 = 1; k1 <= elenk + 1; k1++)
    {
      ...
      for(k2 = 1; k2 <= ln; k2++)
      {
        ...
        if(next[i] != -1) llast[next[i]] = llast[i];
        if(llast[i] != -1)         /* remove i from degree list */
        {
          next[llast[i]] = next[i];
        }
        else
    ...
    for(pk = pk1; pk < pk2; pk++)    /* scan2: degree update */
    {
      ...
      else
      {
        ...
        hhead[h] = i;
        llast[i] = h;      /* save hash of i in last[i] */
      }
    }                                   /* scan2 is done */
    ...
    for(pk = pk1; pk < pk2; pk++)
    {
      ...
      h = llast[i];                      /* scan hash bucket of node i */
      i = hhead[h];
